...
export default class CommitChangesCommand extends Command {
  ...
  async run() {
    ...
    status.all.forEach(file => {
      choices.push({
        name: `${file.path} ${FileNameUtils.getFileChangeType(file.changeType)}`,
        checked: true
      })
    });
    ...
    const commitResult = await GitWrapper.commit(answers.commitMessage, this.getListOfFilesFromPrompt(answers.fileToBeCommitted), answers.skipValidation);
